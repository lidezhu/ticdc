...
func TestDrainLargeTxnInsertsStopsWhenDispatcherRemoved(t *testing.T) {
	...
	strategy := &splitTxnScanStrategy{scanner: scanner, dispatcher: disp}
	interrupted, err := strategy.drainLargeTxnInserts(sess, state)
	require.NoError(t, err)
	require.False(t, interrupted)
...
func TestScanAndMergeEventsSingleUKUpdate(t *testing.T) {
	...
	scanCtx := newTxnScanContext(scanner, sess, merger)
	isInterrupted, err := scanner.scanAndMergeEvents(
		scanCtx,
		newTxnScanStrategy(scanner, stat),
		mockIter,
	)
	events := sess.events
...
func TestScanAndMergeEventsSkipsDeletedTableTxn(t *testing.T) {
	...
	isInterrupted, err := scanner.scanAndMergeEvents(
		scanCtx,
		newTxnScanStrategy(scanner, disp),
		&mockEventIterator{events: []*common.RawKVEntry{rawEvent}},
	)
	require.NoError(t, err)
	require.False(t, isInterrupted)
